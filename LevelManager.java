...
public class LevelManager {
    ...
    public void addTower(int x, int y, int type) {
        ...
        if(type == 0 && this.gold >=50){
            this.gold -= 50; 
            activeTowers.add(new BasicTower( x, y));
        }
        else if(type == 1 && this.gold >=75){
            this.gold -= 75;
            activeTowers.add(new RapidTower( x, y));
        }
        else if(type ==2 && this.gold >= 100){
            this.gold -= 100;
    ...
    public void updateAll() {
        ...
        for(int i = 0; i < activeTowers.size(); i++){
            Tower current = activeTowers.get(i);
            current.tickCooldown();
            if(current.isReadyToFire()){
                Enemy en = current.getClosestEnemy(activeEnemies);
                if(en != null){
        ...
        for(int i = activeEnemies.size()-1; i >= 0; i--){
            Enemy current = activeEnemies.get(i);
            if(current.hasReachedEnd()){
                ...
                this.gold += current.getGoldReward();
                activeEnemies.remove(i);
                
            }
        }
        
        if(enemiesSpawnedThisWave >= enemiesPerWave && activeEnemies.isEmpty()){
            waveNumber++;
            enemiesSpawnedThisWave = 0;
            enemiesPerWave += 3;
        }
